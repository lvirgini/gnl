...
/*   Updated: 2020/03/04 22:13:12 by lvirgini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_next_line(int fd, char **line)
{
	...
	if (fd < 0 || !line || BUFFER_SIZE < 1 || (read(fd, buf, 0) == -1))
		return (-1);
	*line = NULL;//ft_strjoin_max_free_s1(NULL, buf, 0);
	len = 0;
	if (*buf == 0)
		bzero(buf, BUFFER_SIZE + 1);
	while ((end_of_line = ft_strchr_len(buf, '\n')) < 0)
	{
		if ((copy_buf_in_line(line, buf, end_of_line, len) != 1) || ((len = read(fd, buf, BUFFER_SIZE)) < BUFFER_SIZE))
		{
			buf[BUFFER_SIZE] = 0;
